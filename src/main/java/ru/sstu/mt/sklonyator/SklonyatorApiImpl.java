...
public class SklonyatorApiImpl implements SklonyatorApi {
    ...
    private static final String FORMAT_WITH_INFO_BASE_URL = "https://htmlweb.ru/service/api.php?inflect=%s&grammems=%s&info&json";
    ...
    private final String formatWithInfoUrl;
    ...
    public SklonyatorApiImpl(String apiKey) {
        ...
        formatWithInfoUrl = FORMAT_WITH_INFO_BASE_URL + this.apiKey;
    }
    ...
    @Override
    public List<WordFormInfo> transformWithInfo(String word, Collection<RussianGrammem> grammems) throws IOException {
        String grammemsStr = grammems.stream()
                .map(RussianGrammem::getSystemName)
                .collect(Collectors.joining(","));
        JsonNode json = request(String.format(formatWithInfoUrl, word, grammemsStr));
        List<WordFormInfo> results = new ArrayList<>();
        for (int i = 0; ; i++) {
            JsonNode n = json.get(Integer.toString(i));
            if (n == null) break;
            WordFormInfo wordFormInfo = new WordFormInfo();

            wordFormInfo.setWord(n.get("word").asText());
            wordFormInfo.setPos(RussianPos.getBySystemName(n.get("partofspeech").asText()));
            wordFormInfo.setGrammems(StreamSupport.stream(n.get("grammems").spliterator(), false)
                    .map(JsonNode::asText)
                    .map(RussianGrammem::getBySystemName)
                    .filter(Objects::nonNull)
                    .collect(Collectors.toList()));
            results.add(wordFormInfo);
        }
